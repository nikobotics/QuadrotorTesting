...
def do_work():
    ...
    for n in range(DETERMINE_EFFECTIVENESS_TRIALS):
        ...
        if NUM_SENSORS == 1:
            carrierSig = so.get_carrier_signal(SIZE, NoiseType.STEP)
        else:
            carrierSigs = so.get_signal_from_n_sensors(SIZE, NoiseType.STEP, NoiseType.PSEUDO_RAND, NUM_SENSORS)
        ...
        print("Trial effect #" + str(n+1) + ": " + str(round(effect * 100)))
